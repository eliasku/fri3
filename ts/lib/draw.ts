...
export const drawTriangles = (vb: Ptr<void>, vb_size: u32, ib: Ptr<u16>, indices_count: u32, handle: u32): void => {
    ...
    // upload buffers data
    gl.bufferData(GL.ARRAY_BUFFER, new Uint8Array(MEM.buffer, vb, vb_size), GL.STREAM_DRAW);
    gl.bufferData(GL.ELEMENT_ARRAY_BUFFER, new Uint16Array(MEM.buffer, ib, indices_count), GL.STREAM_DRAW);

    // bind buffers to program
    bindAttrib(opaqueProgram._aPosition, 3, vertexByteSize, 0, GL.FLOAT, false);
    bindAttrib(opaqueProgram._aTexCoord, 2, vertexByteSize, 12, GL.FLOAT, false);
    bindAttrib(opaqueProgram._aColorMul, 4, vertexByteSize, 20, GL.UNSIGNED_BYTE, true);
    bindAttrib(opaqueProgram._aColorAdd, 4, vertexByteSize, 24, GL.UNSIGNED_BYTE, true);
