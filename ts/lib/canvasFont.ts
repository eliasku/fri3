import { Texture, createTexture, getSubTexture, uploadTexture } from "./texture.js";
import { GL } from "./base/webgl.js";
...
const createFontCanvas = (size: number): CanvasRenderingContext2D => {
...
const gctx = createFontCanvas(128);
...
export const makeFontAtlas = (family: string, size: number, scale: number, style: FontStyleDef): FontAtlas => {
    ...
    const ctx = createFontCanvas(canvasSize);
    const fallback =
    ...
    const fa: FontAtlas = {
        ...
        _ctx: ctx,
        _texture: texture,
        _textureBox: getSubTexture(texture, 1, 1, 1, 1, 0, 0),
        _nextSheetX: 0,
        _nextSheetY: 0,
        _sheetLineHeight: 0,
        _characters: [],
...
const enlargeFontTexture = (fa: FontAtlas) => {
    ...
    fa._textureBox = getSubTexture(fa._texture, 1, 1, 1, 1);
...
export const drawTextAligned = (
    ...
    drawText(font, text, size, x, y, size, 1, color, alpha);
};
